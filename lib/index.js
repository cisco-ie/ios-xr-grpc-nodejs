...
var telemetry_msg = require(__dirname + '/proto/telemetry_pb.js');
 ...
 * @param {boolean} nounmarshal Indicator to not unmarshal the telemetry data.
 * @return {object} Event emitter of data, error, status, and end.
 */
xrGrpc.prototype.createSubscription = function(reqId, encode, subId, nounmarshal) {
	...
	if (!nounmarshal) {
		...
		stubCall.on('data', function(telemetry) {
			unmarshaller.callback(null, telemetry);
		});
	}
